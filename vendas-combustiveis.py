...
try:
  ...
  for col in colunas_necessarias:
    if col not in df.columns:
      st.error(
          f"Erro: A coluna '{col}' não foi encontrada na base de dados."
      )
      st.stop()
  ...
  with abas[0]:
    ...
    # Tabela formatada no padrão brasileiro
    ...
    with st.expander("💡 Resposta referente a este gráfico (Pergunta 1)"):
      st.markdown(
          "**Resposta:** O **Óleo Diesel** possui o maior volume acumulado de"
          " vendas no país, seguido diretamente pela **Gasolina C**. Juntas,"
          " essas duas categorias lideram amplamente o consumo nacional de"
          " combustíveis."
      )
    ...
    df_linha_geral = df.groupby(["ANO", "PRODUTO"], as_index=False)[
        "VENDAS"
    ].sum()
    ...
    with st.expander("💡 Resposta referente a este gráfico (Pergunta 3)"):
      st.markdown(
          "**Resposta:** A tendência de evolução ao longo dos anos é"
          " **ascendente (de crescimento)** para os principais combustíveis"
          " comercializados, mantendo o Óleo Diesel e a Gasolina C"
          " constantemente no topo do volume anual."
      )
    ...
    df_regiao_geral = (
        df.groupby("GRANDE REGIÃO", as_index=False)["VENDAS"]
        .sum()
    ...
    with st.expander("💡 Resposta referente a este gráfico (Pergunta 2)"):
      st.markdown(
          "**Resposta:** O volume de vendas distribui-se de forma desigual"
          " entre as regiões, concentrando-se fortemente na **Região"
          " Sudeste** (que detém quase metade das vendas), seguida do **Sul** e"
          " **Nordeste**, enquanto **Centro-Oeste** e **Norte** registram as"
          " menores fatias do mercado."
      )
  ...
  for i, prod in enumerate(produtos_unicos):
    with abas[i + 1]:
      ...
      m1.metric("Volume Acumulado do Produto", f"{fmt_br(vol_total_prod)} m³")
      m2.metric("Média Mensal por Estado", f"{fmt_br(vol_med_prod)} m³")
      ...
      st.markdown(f"""
            ...
            Os principais resultados observados são: a região **{reg_maior_nome}** possui o maior valor acumulado de vendas com **{fmt_br(reg_maior_val)} m³**. O gráfico deixa evidente a diferença na quantidade consumida entre a primeira colocada e as demais regiões do país.
            """)

      with st.expander(f"💡 Resposta regional de {prod} (Pergunta 2)"):
        st.markdown(
            f"**Resposta:** Para o combustível **{prod}**, a maior concentração"
            f" de vendas ocorre na **Região {reg_maior_nome}**, registrando o"
            " maior volume total entre todas as regiões comparadas."
        )
      ...
      st.markdown(f"""
            ...
            Os principais resultados observados são: a maior fatia pertence à região **{reg_maior_nome}**, correspondendo a **{fmt_br(pct_maior)}%** das vendas. As fatias restantes destacam a proporção individual de consumo das demais regiões brasileiras.
            """)

      with st.expander(
          f"💡 Resposta de participação percentual de {prod} (Pergunta 2)"
      ):
        st.markdown(
            f"**Resposta:** A Região **{reg_maior_nome}** responde por"
            f" **{fmt_br(pct_maior)}%** de todas as vendas de **{prod}** no"
            " país, evidenciando sua liderança proporcional no mercado desse"
            " produto."
        )
      ...
      df_prod_ano_reg = df_prod.groupby(["ANO", "GRANDE REGIÃO"], as_index=False)[
          "VENDAS"
      ].sum()
      ...
      with st.expander(
          f"💡 Resposta da evolução temporal de {prod} (Pergunta 3)"
      ):
        st.markdown(
            f"**Resposta:** A evolução das vendas de **{prod}** mostra o"
            " histórico do produto ao longo do tempo, mantendo a Região"
            f" **{reg_maior_nome}** no nível superior das vendas anuais."
        )
  ...
  st.error(
      f"⚠️ O arquivo `{CAMINHO_ARQUIVO}` não foi encontrado na raiz do projeto ou"
      " ocorreu um erro na leitura. Certifique-se de que o arquivo esteja com o"
      " nome exato."
  )
